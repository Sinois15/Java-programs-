...
public class Commands{
    ...
    public void sellAll(Player player){
        if(player.getInventory().getSize() != 0){
            for(int i = 0; i < player.getInventory().getSize(); i++){
                if(!player.getInventory().getItem(i).getName().toLowerCase().contains("seed")){
                    ...
                    for(int j = 0; j < shop.getInventory().getSize(); j++){
                        if(player.getInventory().getItem(i).getName().toLowerCase().equals(shop.getItem(j).getName().toLowerCase())){
                            shopId = shop.getItem(j).getId();
                            player.setBalance(player.getBalance() + (shop.getItem(shopId).getPrice() * player.getInventory().getItem(i).getQuantity()));
                            player.getInventory().getItem(i).setQuantity(0);
                        }
                    }                    
                }
            }
            ...
            System.out.println("Successfully Sold Inventory!");
        } else {
            System.out.println("Inventory is empty or only contains seeds, could not sell anything.");
        }
    }
